...
INT PrintHelp(IN CHAR* _Argv0) {
	...
	printf("\t\t\t #######################################\n\n");
	printf("[!] Usage: Founding.exe <Generator Type> <Payload> <Enc/Obf> <Shellcode Execution type> <Optional Flag>\n\n");
	printf("[i] Enc/Obf options allows the shellcode to be represented as an array in various formats like: \n");
	printf("\n");
	printf("\tObfuscation:\n");
	printf("\t	[+] mac                           ::: Mac Addresses  [FC-48-83-E4-F0-E8]\n");
	printf("\t	[+] ipv4                          ::: Ipv4 Addresses [252.72.131.228]\n");
	printf("\t	[+] ipv6                          ::: Ipv6 Addresses [FC48:83E4:F0E8:C000:0000:4151:4150:5251]\n");
	printf("\t	[+] uuid                          ::: UUid Strings   [FC4883E4-F0E8-C000-0000-415141505251]\n");
	printf("\n");
	printf("\tEncryption:\n");
	printf("\t	[+] aes                           ::: Aes Encrypted Shellcode With Random Key And Iv\n");
	printf("\t	[+] rc4                           ::: Rc4 Encrypted Shellcode With Random Key\n");
	printf("\t	[+] xor                           ::: Xor Encrypted Shellcode With Random Key\n\n");
	printf("[i] Shellcode Execution type Options Can Be: \n");
	printf("\n");
	printf("\t        [+] apc		                  ::: Asynchronous Procedure Calls\n");
	printf("\t        [+] early-bird-debug              ::: Asynchronous Procedure Calls with a Remote Debug Process\n");
	printf("\t	[+] early-bird-suspended          ::: Asynchronous Procedure Calls writing using APC on Remote Suspended Process\n");
	printf("\t	[+] enumThreadWindows             ::: Callback function EnumThreadWindows\n");
	printf("\t	[+] local-mapping-inject          ::: Local Mapping and Thread in Suspend State\n");
	printf("\t	[+] early-cascade	          ::: Hooks ntdll!SE_DllLoaded to execute the payload\n");
	printf("\t	[+] fibers	                  ::: Fibers executes by switching execution contexts without creating new threads\n");
	printf("\t	[+] process-hypnosis	          ::: Create child process in debug mode, detach debugger, and execute payload\n");
	printf("\t	[+] tp-alloc	                  ::: Use Thread Pool API (TpAllocWait/TpSetWait) to queue shellcode execution\n");
	printf("\t\t[+] local-hollowing	          ::: Duplicates thread to recreate and run the PE in suspended main thread (RAW generator only with a .exe)\n\n");
	printf("[i] Optional Flags: \n");
	printf("\n");
	printf(" INDIRECT-SYSCALLS:\n");
	printf("\t	[+] --hells-hall                  ::: Change all implementation to Indirect Syscalls (HellsHall) including optional flags\n");
	printf("\t	[+] --syswhispers                 ::: Change all implementation to Indirect Syscalls (SysWhispers3) including optional flags\n");
	printf("\n");
	printf("          COMPILER:\n");
	printf("\t	[+] --llvm                        ::: Use clang-LLVM obfuscation to evade static analysis, some implementations may fail\n");
	printf("\n");
	printf("\t      AMSI:\n");
	printf("\t	[+] --amsi-opensession            ::: Patch AmsiOpenSession to return invalid argument\n");
	printf("\t	[+] --amsi-scanbuffer             ::: Patch AmsiScanBuffer to return invalid argument\n");
	printf("\t	[+] --amsi-signature              ::: Patch AmsiSignature to return invalid string corrupting the signature value\n");
	printf("\t	[+] --amsi-codetrust              ::: Patch WldpQueryDynamicCodeTrust to return invalid argument\n");
	printf("\n");
	printf("\t    UNHOOK:\n");
	printf("\t	[+] --unhooking-createfile        ::: Unhook ntdll.dll mapped with CreateFileMappingA\n");
	printf("\t	[+] --unhooking-knowndlls         ::: Unhook ntdll.dll from KnownDlls directory\n");
	printf("\t	[+] --unhooking-debug             ::: Unhook ntdll.dll copying the new NTDLL from a new debug process\n");
	printf("\t	[+] --hookchain	                  ::: Modifies the IAT to reroute function calls, allowing it to intercept and handle them\n");
	printf("\n");
	printf("\t       ETW:\n");
	printf("\t	[+] --etw-eventwrite              ::: Patch EtwEventWriteFull, EtwEventWrite and EtwEventWriteEx\n");
	printf("\t	[+] --etw-trace-event             ::: Patch NtTraceEvent\n");
	printf("\t	[+] --etw-peventwritefull         ::: Patch private function EtwpEventWriteFull to return invalid parameters\n");
	printf("\n");
	printf("\t   SANDBOX:\n");
	printf("\t	[+] --api-hammering	          ::: Creates a random file, reads/writes random data, delaying execution for 10 sec\n");
	printf("\t	[+] --delay-mwfmoex	          ::: Use MsgWaitForMultipleObjectsEx delaying execution for 10 sec\n");
	printf("\t	[+] --ntdelay	                  ::: Use NtDelayExecution delaying execution for 10 sec\n");
	printf("\t	[+] --fibonacci	                  ::: Calculate Fibonacci delaying execution for 10 sec\n");
	printf("\t	[+] --mouse-clicks	          ::: Logs clicks for 20 seconds, if fewer than 1 click it assumes as sandboxed environment\n");
	printf("\t	[+] --resolution	          ::: Checks resolution for sandbox environments\n");
	printf("\t	[+] --processes	                  ::: Checks if the system is running less than 50 processes it assumes as sandboxed environment\n");
	printf("\t	[+] --hardware	                  ::: Checks if the system is running less of 2 processors, 2 gb ram and 2 usbs mounted it assumes as sandboxed environment\n");
	printf("\n");
	printf("   PAYLOAD-CONTROL:                       \n");
	printf("\t	[+] --check-running	          ::: Check if the executable is already running, if it is, prevent duplicate execution.\n");
	printf("\t	[+] --self-delete	          ::: Ensure the payload deletes itself during execution, if deletion fails, deletes file content reducing its size to zero bytes.\n");
	printf("\n");
	printf("\t      MISC:\n");
	printf("\t	[+] --dll [export_name]           ::: Create a DLL with optional export function name (default: runme), this implementation runs rundll32 in background\n");
	printf("\t	[+] --dll-stealthy [export_name]  ::: Create a DLL with optional export function name (default: runme), stealthier but some implementations may fail\n");
	printf("\t	[+] --service                     ::: Create a executable to be run as a service\n");
	printf("\t\t[+] --inflate [number]            ::: Inflate the executable with random portuguese words to increase his size\n");
	printf("\t	[+] --sign [pfx] [pass]           ::: Sign the final executable with a certificate (password is optional)\n");
	printf("\t	[+] --no-window                   ::: Run without opening a terminal window\n");
	printf("\t	[+] --no-print                    ::: Run without printing any output, remove all printfs from implementation\n");
	printf("\t\t[+] --decoy [file]                ::: Embed a decoy file (e.g. PDF) to be executed alongside the payload\n");
	printf("\n\n");
	//system("PAUSE");
	return -1;
}
